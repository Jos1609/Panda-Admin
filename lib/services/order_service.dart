...
class OrderService {
    ...
    try {
      ...
      await _firestore.runTransaction((transaction) async {
        ...
        transaction.update(orderRef, {
          ...
          'statusHistory': order.statusHistory.map((log) => log.toMap()).toList(),
        });
      });
    } catch (e) {
  ...
  Future<Map<String, dynamic>> getOrderStats(
    ...
    try {
      ...
      final orders = querySnapshot.docs.map((doc) => DeliveryOrder.fromFirestore(doc)).toList();

      return {
        'totalOrders': orders.length,
        // ignore: avoid_types_as_parameter_names
        'totalRevenue': orders.fold(0.0, (sum, order) => sum + order.total),
        'avgDeliveryTime': _calculateAverageDeliveryTime(orders.cast<DeliveryOrder>()),
        'cancelRate': _calculateCancelRate(orders.cast<DeliveryOrder>()),
        'statusDistribution': _getStatusDistribution(orders.cast<DeliveryOrder>()),
      };
    } catch (e) {
  ...
  double _calculateAverageDeliveryTime(List<DeliveryOrder> orders) {
    final deliveredOrders = orders.where((order) => order.status == OrderStatus.delivered);
  ...
  double _calculateCancelRate(List<DeliveryOrder> orders) {
    ...
    final cancelledOrders = orders.where((order) => order.status == OrderStatus.cancelled);
